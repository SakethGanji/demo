...
/** Summary stats from a trace tree */
...
/** Walk the tree and collect summary stats */
export function getTraceStats(node: TraceNode): TraceStats {
  ...
  function walk(n: TraceNode) {
    switch (n.kind) {
      case 'agent':
        stats.totalDuration = n.duration;
        stats.iterations = n.iterations || 0;
        break;
      case 'tool_call':
        stats.toolCalls++;
        if (n.isError) stats.toolErrors++;
        break;
      case 'spawn':
        stats.spawns++;
        break;
      ...
      case 'reflect':
        ...
        break;
      case 'validation':
        stats.validationStatus = n.status;
        break;
    }
    if ('children' in n && Array.isArray((n as { children?: unknown }).children)) {
      for (const child of (n as { children: TraceNode[] }).children) {
        walk(child);
      }
    }
  }
