...
// Generic over the argument tuple rather than over the whole function type:
// `T extends (...args: unknown[]) => void` looks permissive but is unsatisfiable
// for any real callback, because parameters are contravariant — a
// `(key: string, ...) => void` cannot accept an `unknown` first argument.
...
interface NodePropertyTypeOptions {
  ...
  // Sent for `filePath` properties; consumed by FilePathField.
  extensions?: string;
}
