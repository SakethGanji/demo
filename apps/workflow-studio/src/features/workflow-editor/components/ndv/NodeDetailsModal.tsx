...
export default function NodeDetailsModal() {
  ...
  // Base UI has no `onEscapeKeyDown` on the popup — closes are reported through
  // the root's `onOpenChange` with a `reason`, and `cancel()` suppresses them.
  const handleOpenChange = useCallback(
    (open: boolean, eventDetails: { reason: string; cancel: () => void }) => {
      if (open) return;
      if (eventDetails.reason === 'escape-key' && isEditingName) {
        eventDetails.cancel();
        handleCancelEdit();
        return;
      }
    ...
    [isEditingName, handleCancelEdit, closeNDV],
  );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogPortal>
        <DialogOverlay />
        <DialogContent showCloseButton={false} className="!max-w-[90vw] !w-[90vw] !h-[85vh] !p-0 !gap-0 flex flex-col !bg-background">
