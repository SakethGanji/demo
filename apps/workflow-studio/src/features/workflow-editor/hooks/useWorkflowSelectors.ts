  ...
  return useWorkflowStore(
    useCallback((s) => {
      ...
      for (const n of s.nodes) {
        // `nodes` is typed as bare `Node[]` in the store, so `data` widens to `{}`.
        const data = n.data as WorkflowNodeData;
        const nodeName = data?.name || data?.label;
        const output = s.executionData[n.id]?.output;
        if (nodeName && output?.items) {
          result[nodeName] = output.items as Record<string, unknown>[];
        }
      }
      return result;
    }, []),
