...
function RootLayout() {
  ...
  const isDataRoute = matchRoute({ to: '/data', fuzzy: true })
  const isCatalogRoute = matchRoute({ to: '/catalog', fuzzy: true })
  ...
  const isFullScreen =
    isEditorRoute || isBuilderRoute || isProjectsRoute || isDataRoute || isCatalogRoute

  // Full takeover layout (editor, builder, projects, data, catalog, landing)
