...
from conftest import SAMPLE_CSV, auth, create_team_user, rid
...
async def test_rbac_viewer_is_read_only(client, admin_id):
    uid, team_id = await create_team_user(client, admin_id, "viewer")
...
async def test_rbac_team_isolation_hides_existence(client, admin_id):
    ...
    uid_a, team_a = await create_team_user(client, admin_id, "editor")
    uid_b, team_b = await create_team_user(client, admin_id, "editor")
    with open(SAMPLE_CSV, "rb") as f:
...
async def test_file_path_source_requires_superuser(client, admin_id):
    ...
    uid, _ = await create_team_user(client, admin_id, "editor")
    r = await client.post(
        "/api/v1/sample",
        headers=auth(uid),
...
async def test_audit_requires_superuser(client, admin_id):
    ...
    uid, tid = await create_team_user(client, admin_id, "admin")  # team admin, not superuser
    assert (await client.get("/api/v1/audit", headers=auth(uid))).status_code == 403


async def test_audit_records_request_duration(client, admin_id):
    """Every audited request carries its latency (duration_ms)."""
    h = auth(admin_id)
    r = await client.post("/api/v1/teams", headers=h, json={"name": f"lat-{rid()}"})
    assert r.status_code == 201
    entries = (await client.get("/api/v1/audit", params={"limit": 5},
                                headers=h)).json()["items"]
    newest = entries[0]
    assert newest["path"] == "/api/v1/teams"
    assert isinstance(newest["duration_ms"], int) and newest["duration_ms"] >= 0
