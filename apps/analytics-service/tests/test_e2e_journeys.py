...
def _make_workbook(path, *, quarter_col: bool, second_sheet: str, extra_revenue_row: bool):
...
async def _new_user(client, admin_id, team_id, role):
    """Provision a user in *team_id* with *role*, the way an admin console would."""
    h = auth(admin_id)
    r = await client.post("/api/v1/auth/users", headers=h,
                          json={"email": f"{role}-{rid()}@bank.com", "name": role.title(),
                                "team_id": team_id})
    assert r.status_code == 201, r.text
    uid = r.json()["id"]
    if role != "viewer":
        r = await client.patch(f"/api/v1/teams/{team_id}/members/{uid}", headers=h,
                               json={"role": role})
...
async def test_journey_team_workbook_lifecycle(client, admin_id, tmp_path):
    ...
    tid = team["id"]
    team_admin = await _new_user(client, admin_id, tid, "admin")
    editor = await _new_user(client, admin_id, tid, "editor")
    viewer = await _new_user(client, admin_id, tid, "viewer")

    other_team = (await client.post("/api/v1/teams", headers=h_admin,
                                    json={"name": f"other-{rid()}"})).json()
    outsider = await _new_user(client, admin_id, other_team["id"], "editor")
    ...
    _make_workbook(v1_file, quarter_col=False, second_sheet="Expenses", extra_revenue_row=False)
    ...
    _make_workbook(v2_file, quarter_col=True, second_sheet="Spending", extra_revenue_row=True)
