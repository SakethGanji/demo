...
from conftest import DEFAULT_TEAM_ID, XLSX_MIME, auth, make_workbook, upload_file
...
async def excel_dataset(client, admin_id, tmp_path):
    """A dataset with two Excel versions (see conftest.make_workbook)."""
    v1 = tmp_path / "v1.xlsx"
    v2 = tmp_path / "v2.xlsx"
    make_workbook(v1)
    make_workbook(v2, quarter_col=True, second_sheet="Spending",
                  extra_revenue_row=True)
    up = await upload_file(client, admin_id, v1, name="book.xlsx",
