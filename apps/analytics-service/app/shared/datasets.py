...
async def get_version_sheet_rows(ver: dict) -> list[dict]:
    ...
    return [
        {
            "id": None,
            "logical_sheet_id": None,
            "sheet_key": normalize_sheet_key(s["name"]),
            "sheet_name": s["name"],
            # Legacy JSONB versions predate logical identity, so there is no
            # current-name hop to take; the keys are present so callers never
            # have to branch on the row's provenance.
            "logical_sheet_key": None,
            "logical_sheet_name": None,
            "sheet_index": i,
...
async def resolve_version_sheet_row(ver: dict, sheet: str | None) -> dict | None:
    """Resolve a (version, sheet?) pair to its sheet row.
    ...
    problem listing the available sheets. Returns None only for legacy
    versions with no sheet rows at all (callers fall back to ``ver["path"]``).
    """
...
async def resolve_version_sheet_path(ver: dict, sheet: str | None) -> str:
    """Resolve a (version, sheet?) pair to a parquet path (see resolve_version_sheet_row)."""
...
def _find_sheet(sheets: list[dict], sheet: str) -> dict | None:
    """Match a sheet by name, then key, then the logical sheet's CURRENT name.

    The first two hops are the version's own frozen identifiers and always win:
    a version that physically contains a sheet called *sheet* must resolve to
    that sheet, never to some other row whose logical identity answers to the
    same name. Only when nothing in the version matches do we take ROADMAP §1's
    second hop and compare against ``dataset_sheets.display_name`` /
    ``current_sheet_key`` — the name every screen shows today.

    That last hop is what lets a confirmed rename stay invisible to callers:
    after "Expenses" → "Operating Costs" the sheet picker only offers the new
    name, and pinning an older version must not turn that name into a 404. The
    reverse is deliberately NOT true — a retired name does not resolve against
    a later version, because the key it freed can legitimately be taken by a
    different sheet.
    """
    for r in sheets:
    ...
    for r in sheets:
        if r["sheet_key"] == sheet:
            return r
    for r in sheets:
        if r.get("logical_sheet_name") == sheet:
            return r
    for r in sheets:
        if r.get("logical_sheet_key") == sheet:
            return r
    return None
