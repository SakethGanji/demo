...
class InvalidFileError(ValueError):
    """The uploaded file could not be parsed in its declared format.

    A user error, not a server fault — the upload API maps it to a
    problem+json 400 (``invalid-file``) instead of a 500.
    """


def _scrub_paths(message: str) -> str:
    """Strip server filesystem paths out of a parser error before it is shown.

    Engine errors embed the absolute staging path (``/tmp/accelerator/tus_uploads/
    _staging/<uuid>_raw.csv``) — sometimes double-quoted in the message, and again
    unquoted inside a suggested ``read_csv_auto('…')`` fix. That is internal
    deployment detail with no value to the uploader, so reduce every absolute
    path to its basename regardless of how it is quoted.
    """
    return re.sub(r"/(?:[^\s'\"/]+/)+([^\s'\"/]+)", r"\1", message)


class NoMatchingSheetsError(ValueError):
    """``include_sheets`` selected nothing: no such sheet, or all of them empty.

    Also a user error, and deliberately NOT an :class:`InvalidFileError` — the
    file parsed fine, the selection was wrong, and the two map to different
    problem codes (``sheet-not-found`` vs ``invalid-file``). Carrying
    ``sheets`` (what the workbook actually holds) on the exception is what lets
    a caller answer with a picker instead of a bare message; before this the
    converter raised a bare ``ValueError``, so every caller had to either
    re-type it by guesswork or let it surface as a 500.
    """

    def __init__(self, message: str, *, sheets: list[str] | None = None):
    ...
    if suffix in (".csv", ".parquet"):
        ...
        try:
            ...
            raise InvalidFileError(
                f"File could not be parsed as {suffix.lstrip('.')}: "
                f"{_scrub_paths(str(e))}") from e
        conn.execute(f"CREATE VIEW df AS SELECT * FROM read_parquet('{dest_path}')")

    elif suffix in (".xlsx", ".xls"):
        try:
            xls = pd.ExcelFile(source_path, engine="openpyxl")
        except Exception as e:  # BadZipFile / InvalidFileException / parser errors
            conn.close()
            raise InvalidFileError(
                f"File could not be parsed as Excel: {_scrub_paths(str(e))}") from e
