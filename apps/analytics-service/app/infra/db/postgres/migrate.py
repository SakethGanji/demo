...
Commands:
  python -m app.infra.db.postgres.migrate                           # apply pending migrations (default)
  python -m app.infra.db.postgres.migrate apply                     # same as above
  python -m app.infra.db.postgres.migrate status                    # show applied/pending migrations
  python -m app.infra.db.postgres.migrate dry-run                   # show what would be applied
  python -m app.infra.db.postgres.migrate rollback                  # rollback the last applied migration
  python -m app.infra.db.postgres.migrate rollback 20260307191500   # rollback to a specific version
  python -m app.infra.db.postgres.migrate reset                     # drop schema and re-apply all from scratch
  python -m app.infra.db.postgres.migrate new "create_users"        # create a new migration file

Connection overrides (for remote access):
  python -m app.infra.db.postgres.migrate apply --admin-user postgres --admin-password secret
  python -m app.infra.db.postgres.migrate apply --host db.prod --port 5432 --dbname accelerator --admin-user postgres --admin-password secret
"""
...
from app.infra.db.postgres.session import resolve_database_url

MIGRATIONS_DIR = Path(__file__).parent / "migrations"
LOCK_ID = 839202  # project-specific advisory lock ID (different from workflow-engine)
...
def _get_conn():
    ...
    if args and (args.admin_user or args.host or args.port or args.dbname):
        ...
        if not url:
            ...
            print("  Set ACCELERATOR_DATABASE_URL or individual ACCELERATOR_DB_* vars.")
            sys.exit(1)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m app.infra.db.postgres.migrate",
