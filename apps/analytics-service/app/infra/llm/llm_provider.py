...
def _get_env(key: str) -> str | None:
    """Read a config value, checking ANALYTICS_ prefix first, then raw."""
    val = os.environ.get(f"ANALYTICS_{key}") or os.environ.get(key)
    if val:
        return val
    try:
...
# top_p, or top_k to one of these returns a 400, so the caller's temperature is
# dropped rather than forwarded. Determinism on these models comes from
# prompting and low effort, not from temperature=0.
...
def _accepts_temperature(model: str) -> bool:
    return not model.startswith(_NO_SAMPLING_PARAM_MODELS)
...
async def _call_anthropic(
    ...
    **kwargs: Any,
    ...
    if api_tools:
        ...
        # Forcing a specific tool is how callers get structured output from
        # Claude; without this passthrough the model is free to answer in prose
        # and every structured caller has to handle a text fallback.
        tool_choice = kwargs.get("tool_choice")
        if isinstance(tool_choice, str):
            tool_choice = {"auto": {"type": "auto"},
                           "required": {"type": "any"},
                           "any": {"type": "any"},
                           "none": {"type": "none"}}.get(tool_choice)
        if tool_choice:
...
async def get_embeddings_batch(
    ...
    Batches requests to avoid API limits. Supports OpenAI and Gemini providers.
            ...
            try:
                ...
                if _is_retryable(exc) and attempt < _MAX_RETRIES - 1:
                    ...
                    logger.warning(
                        ...
                        i // batch_size, attempt + 1, _MAX_RETRIES, delay, exc,
                    )
...
async def call_llm(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        tools: List of tools — each can be a Python callable or a dict
               with name/description/parameters.
        **kwargs: Extra options: max_tokens, response_format, user.
