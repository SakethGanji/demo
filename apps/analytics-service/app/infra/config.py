...
class Settings(BaseSettings):
    ...
    # Storage
    storage_dir: Path = Path("/tmp/accelerator")  # base for datasets/, artifacts/
    ...
    # Background job worker (pulls the `jobs` table). Started in the app
    # lifespan; the in-process ASGI test client does NOT run the lifespan, so
    # tests drive the worker deterministically via worker.run_pending_jobs_once.
    job_worker_enabled: bool = True
    job_worker_poll_seconds: float = 1.0

    # LLM settings — used by app/infra/llm/ (currently the embeddings gateway
    # in data_accelerator/services/methods.py::get_embeddings_batch).
