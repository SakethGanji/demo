...
_RULE_COLS = """id::text, dataset_id::text, name, description, scope_type,
                sheet_selector, logical_sheet_id::text, column_selector,
    ...
    async with async_session_factory() as s:
        row = (await s.execute(
            text(f"""
                INSERT INTO quality_rules
                    (dataset_id, name, description, scope_type, sheet_selector,
                     logical_sheet_id, column_selector, rule_type, parameters,
                     severity, enabled, created_by)
                VALUES (:did, :name, :description, :scope_type, :sheet_selector,
                        :logical_sheet_id, :column_selector, :rule_type,
            ...
            {"did": dataset_id, "created_by": created_by,
             ...
             "logical_sheet_id": fields.get("logical_sheet_id"),
...
_MUTABLE = {"name", "description", "sheet_selector", "logical_sheet_id",
            "column_selector", "parameters", "severity", "enabled"}
        ...
        for r in results:
            await s.execute(
                text("""
                    INSERT INTO validation_rule_results
                        (validation_run_id, rule_id, rule_name, rule_type, scope_type,
                         ...
                         failure_count, message, failure_artifact_id)
                ...
                {"rid": run_id, **{k: r.get(k) for k in
                 ...
                 "failure_artifact_id": r.get("failure_artifact_id")},
            )
        row = (await s.execute(
            text(f"""
...
async def fail_run(run_id: str, error: str) -> None:
    ...
    ended" rather than "fail it". ``validate_version`` fails the run from one
    ``except`` that used to span the announcement as well as the validation, so
    anything raised after ``complete_run`` demoted a run whose rule results were
    already persisted. The results stay in ``validation_rule_results`` either
    way; it is only the status that would be a lie.
...
async def list_run_results(run_id: str) -> list[dict]:
    async with async_session_factory() as s:
        rows = (await s.execute(
            text("""
                SELECT r.id::text, r.rule_id::text, r.rule_name, r.rule_type,
                       r.scope_type, r.sheet_selector, r.column_selector,
                       r.severity, r.status, r.failure_count, r.message,
                       r.failure_artifact_id::text,
                       -- The failing rows live in the object store; surface the
                       -- filename so callers can fetch them via /samples.
                       regexp_replace(a.storage_key, '^.*/', '') AS failure_sample_file
                FROM validation_rule_results r
                LEFT JOIN artifacts a ON a.id = r.failure_artifact_id
                WHERE r.validation_run_id = :rid
