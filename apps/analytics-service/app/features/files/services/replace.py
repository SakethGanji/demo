...
async def replace_sheet(ds: dict, sheet_name: str, file: UploadFile, principal) -> dict:
    ...
    try:
        ...
        # STARLETTE's HTTPException, so `except fastapi.HTTPException` is a
        # sibling class that does not match it: an actionable 4xx raised in
        # here (sheet-selection-required, unknown-column, sql-error …) fell
        # through to the generic branch and reached the caller as a 500
        # "Sheet replacement failed: 400: …", losing both its status and its
        # machine-readable ``code``. FastAPI's own HTTPException is a subclass
        # of Starlette's, so it still takes this branch — the catch is widened,
        # never narrowed. Unlike ``execute_definition`` there is no
        # stuck-state half to this bug: both branches already fail the version,
        # so only the client-facing error was ever wrong.
