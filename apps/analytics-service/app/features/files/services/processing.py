...
class UploadRejectedError(ValueError):
    """The content scanner refused the upload.

    A user/content error, not a server fault — the upload API maps it to a
    problem+json 400 (``upload-rejected``), the same way the sheet-replace
    path already does, instead of a 500 that reads like an outage.
    """


class SheetSelectionError(ValueError):
    """``include_sheets`` named no sheet the workbook actually has.

    Typing this is what lets the API answer "you asked for a sheet that isn't
    there" with a 400 the UI can attach to the sheet picker, rather than the
    500 a bare ``ValueError`` produced.
    """


# error_kind -> the classes that mean "the caller's fault, answer 400".
# Order matters: every entry is a ValueError subclass, so the first match wins.
_USER_ERROR_KINDS: tuple[tuple[type[Exception], str], ...] = (
    (UploadRejectedError, "upload-rejected"),
    (SheetSelectionError, "sheet-not-found"),
    (InvalidFileError, "invalid-file"),
)


def classify_processing_error(exc: BaseException) -> str:
    """Machine-readable kind for a failed ingest.

    ``processing-error`` is the only kind that means "server fault"; everything
    else is something the uploader can fix and is surfaced as a 400.
    """
    for kind_cls, kind in _USER_ERROR_KINDS:
        if isinstance(exc, kind_cls):
            return kind
...
async def process_uploaded_file_async(
    ...
    *,
    ...
    # Create a job record. The import belongs to the team that owns the
    # dataset, not to whatever `create_job` defaults to: /jobs is team-scoped
    # (`team_id = ANY(:tids)`), so an unattributed import row is invisible to
    # the team that uploaded the file and visible to the seeded Default team.
    ...
    try:
        ...
        if not scan.ok:
            raise UploadRejectedError(
                f"Upload rejected by scanner: {scan.reason or 'infected'}")
        ...
        # Raw uploaded byte count, persisted so the status poller's DB fallback
        # reports the SAME quantity the warm cache does (which holds the raw
        # upload size). Without this the fallback returned the parquet size,
        # so a completed upload's "N bytes uploaded" changed after a restart.
        ...
        with tempfile.TemporaryDirectory(prefix="accel_build_") as build_dir:
            ...
            try:
                ...
                # Narrow on purpose: the only bare ValueError the converter
                # raises is "include_sheets matched no non-empty sheets", and
                # it can only be reached when the caller opted into sheet
                # filtering. Re-typing it here (rather than in shared code) is
                # what turns a misspelled sheet name into a 400 that names the
                # workbook's real sheets instead of a 500.
                if include_sheets is not None:
                    raise SheetSelectionError(str(e)) from e
                raise
        ...
        # size_bytes is likewise the TOTAL across sheets, matching row_count and
        # the sheet-replace path (replace.py sums per-sheet sizes). The bare
        # canonical parquet size undercounts a multi-sheet workbook by the size
        # of every non-default sheet — a figure the catalog, version list, and
        # search all surface, so a UI would show wildly wrong per-version sizes.
        total_size_bytes = sum(r["size_bytes"] or 0 for r in sheet_rows)
        ...
        processing_status[status_key].update(
            status="error", error=str(e), error_kind=classify_processing_error(e),
        )
        if version_id:
